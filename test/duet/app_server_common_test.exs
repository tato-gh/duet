defmodule Duet.AppServerCommonTest do
  ...
  test "decode_and_process/4 decodes valid UTF-8 JSON lines" do
    ...
    state =
      AppServerCommon.decode_and_process(line, %{seen: nil}, fn msg, st ->
        %{st | seen: msg}
      end, "[test]")
  ...
  test "decode_and_process/4 normalizes invalid UTF-8 latin1 bytes before JSON decode" do
    latin1_json = [~s|{"method":"echo","params":{"text":"|, <<0xE9>>, ~s|"}}|] |> IO.iodata_to_binary()

    state =
      AppServerCommon.decode_and_process(latin1_json, %{seen: nil}, fn msg, st ->
        %{st | seen: msg}
      end, "[test]")
